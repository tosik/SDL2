...
SDL_Texture *
SDL_CreateTextureSpecifiedD3DFragmentShader(SDL_Renderer * renderer, Uint32 format, int access, int w, int h, unsigned int shader_id)
{
    SDL_Texture *texture;
    ...
    if (IsSupportedFormat(renderer, format)) {
        if (renderer->CreateTextureSpecifiedD3DFragmentShader(renderer, texture, shader_id) < 0) {
            SDL_DestroyTexture(texture);
            return NULL;
        }
    } else {
        texture->native = SDL_CreateTextureSpecifiedD3DFragmentShader(renderer,
                                GetClosestSupportedFormat(renderer, format),
                                access, w, h, shader_id);
        if (!texture->native) {
            SDL_DestroyTexture(texture);
            return NULL;
        }
